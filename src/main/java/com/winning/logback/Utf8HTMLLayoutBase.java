...
public abstract class HTMLLayoutBase<E> extends LayoutBase<E> {
    ...
    public void start() {
        ...
        try {
            Parser ex = new Parser(this.pattern);
            ex.setContext(this.getContext());
            Node t = ex.parse();
            this.head = ex.compile(t, this.getEffectiveConverterMap());
    ...
    public Map<String, String> getEffectiveConverterMap() {
        HashMap effectiveMap = new HashMap();
        Map defaultMap = this.getDefaultConverterMap();
        if (defaultMap != null) {
        ...
        if (context != null) {
            Map contextMap = (Map) context.getObject("PATTERN_RULE_REGISTRY");
            if (contextMap != null) {
